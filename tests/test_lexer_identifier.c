...
cam_test_result_t main(void) {
  ...
  // Checks
  wil_lexer_token_t *token1 = cam_type_get_dyn_arr(&context.tokens, 0);
  wil_lexer_token_dump_none(token1);
  CAM_TEST_ASSERT(token1->type == WIL_LEXER_TOKEN_KW_RETURN);
  ...
  wil_lexer_token_dump_str(token2);
  CAM_TEST_ASSERT(token2->type == WIL_LEXER_TOKEN_IDENTIFIER);
}
